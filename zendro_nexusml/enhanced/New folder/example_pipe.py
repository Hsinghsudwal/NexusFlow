...
# Create nodes
node_1 = Node('Node 1', task_1)
node_2 = Node('Node 2', task_2)
node_3 = Node('Node 3', task_3)

# Define dependencies (if any)
node_2.add_dependency(node_1)  # Node 2 depends on Node 1
node_3.add_dependency(node_2)  # Node 3 depends on Node 2

# Create the pipeline
pipeline = Pipeline('Sample Pipeline', 'A simple sample pipeline with sequential/parallel execution.')

# Add nodes to the pipeline
pipeline.add_node(node_1)
pipeline.add_node(node_2)
pipeline.add_node(node_3)

# Set the pipeline mode (sequential or parallel)
pipeline.set_execution_mode('sequential')  # Change to 'parallel' if desired
