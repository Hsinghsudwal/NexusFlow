...
class Step:
    ...
    def __init__(self, function: Optional[Callable] = None, inputs: Optional[List[str]] = None,
                 ...
                 tags: Optional[List[str]] = None, config=None):
    ...
    def __call__(self, *args, **kwargs):
        ...
        def wrapper(func):
            return Step(function=func, inputs=self.inputs, outputs=self.outputs, config=self.config, name=self.name, tags=self.tags)
        return wrapper
    ...
    def execute(self, input_artifacts: Dict, config: Optional[Dict] = None):
        ...
        if self.function:
            ...
            result = self.function(*input_artifacts.values(), **(config or {}))
...
class Pipeline:
    ...
    def run(self, input_artifacts: Dict, config: Optional[Dict] = None):
        ...
        try:
            if self.executor:
                self._run_in_parallel(sorted_steps, input_artifacts, config)
            else:
                self._run_sequentially(sorted_steps, input_artifacts, config)
        except Exception as e:
            logger.error(f"Error during pipeline execution: {e}")
            raise

    def _run_sequentially(self, sorted_steps: List[str], input_artifacts: Dict, config: Optional[Dict] = None):
    ...
    def _run_in_parallel(self, sorted_steps: List[str], input_artifacts: Dict, config: Optional[Dict] = None):
