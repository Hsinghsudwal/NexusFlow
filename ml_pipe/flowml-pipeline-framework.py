...
class Stack:
    ...
    def register_node(self, func: Callable, name: Optional[str] = None, 
                      stage: Optional[int] = None, dependencies: Optional[List[str]] = None) -> Node:
        """Register a function as a node in the pipeline."""
        if hasattr(func, '_node_metadata'):
            # Use metadata from decorator if available
            metadata = func._node_metadata
            node_name = name or metadata["name"]
            node_stage = stage or metadata["stage"]
            node_dependencies = dependencies or metadata["dependencies"]
        else:
            node_name = name or func.__name__
            node_stage = stage or 0
            node_dependencies = dependencies or []
            
        node = Node(func, node_name, node_stage, node_dependencies)
        self.nodes.append(node)
        return node
        
    def run(self) -> str:
        """Execute all nodes in the stack in order of their stage."""
        import time
        ...
        # Keep track of completed nodes and their results
        completed_nodes = set()
        results = {}
        ...
        try:
            for node in sorted_nodes:
                ...
                if not all(dep in completed_nodes for dep in node.dependencies):
                    missing = [dep for dep in node.dependencies if dep not in completed_nodes]
                    raise ValueError(f"Node '{node.name}' has unsatisfied dependencies: {missing}")
                
                logging.info(f"Running node '{node.name}' (stage {node.stage})")
                
                # Execute the node, passing in the config, artifact_store, and run_id
                node_start_time = time.time()
                result = node(config=self.config, artifact_store=self.artifact_store, 
                             run_id=run_id, **{k: results[k] for k in node.dependencies if k in results})
...
class DataIngestion:
    ...
    def data_ingestion(self, path: str, config: Config, artifact_store: ArtifactStore, 
...
class TrainingPipeline:
    ...
    def __init__(self, data_path: str, config_path: str):
        ...
        # Register nodes with the stack
        self.stack.register_node(
...
# Create config.yml from the sample_config generated
# Then run:
python pipeline.py --data your_data.csv --config config/config.yml