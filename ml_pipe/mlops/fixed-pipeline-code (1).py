...
class ModelTrainingTask(PipelineTask):
    def __init__(self, config, artifact_store):
    ...
    def execute(self, inputs=None):
        ...
        if not inputs or 'processed_train' not in inputs:
            raise ValueError("Required input 'processed_train' not provided to ModelTrainingTask")
        
        train_data = inputs['processed_train']
        test_data = inputs['processed_test']
        ...
        model_path = self.artifact_store.save_artifact(
            model, subdir="models", name="trained_model.pkl"
        )
        
        logging.info(f"Model training completed. Model saved to {model_path}")
        
        # Return model path and trained model
        return {
...
class ModelEvaluationTask(PipelineTask):
    def __init__(self, config, artifact_store):
    ...
    def execute(self, inputs=None):
        ...
        if not inputs or 'model' not in inputs or 'X_test' not in inputs or 'y_test' not in inputs:
            raise ValueError("Required input(s) 'model', 'X_test', or 'y_test' not provided to ModelEvaluationTask")
        ...
        metrics_path = self.artifact_store.save_artifact(
            metrics, subdir="metrics", name="evaluation_metrics.json"
        )
...
class TrainingPipeline:
    ...
    def run(self) -> Tuple[str, StackPipeline]:
        ...
        model_training_task = ModelTrainingTask(self.config, artifact_store)
        model_evaluation_task = ModelEvaluationTask(self.config, artifact_store)
        
        # Add tasks to pipeline
        pipe.add_task(data_ingestion_task)
        pipe.add_task(data_processing_task)
        pipe.add_task(model_training_task)
        pipe.add_task(model_evaluation_task)
