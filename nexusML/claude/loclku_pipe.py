...
class Stack:
    ...
    def __init__(self, name: str, max_workers: int = 4, run: str = 'local', scale: str = 'docker'):
        self.name = name
    ...
    def deploy(self, model: Any, config: Dict) -> bool:
        ...
        if self.run == 'local':
            ...
            deployment_success = self.deploy_to_cloud(model, config)
        return deployment_success
    
    def deploy_to_cloud(self, model: Any, config: Dict) -> bool:
        """Deploy model to cloud environment"""
        if self.scale == 'docker':
            return self.deploy_with_docker(model, config)
        elif self.scale == 'kubernetes':
            return self.deploy_with_kubernetes(model, config)
        else:
            raise ValueError(f"Unsupported scaling option: {self.scale}")
    
    def deploy_with_docker(self, model: Any, config: Dict) -> bool:
        """Deploy model using Docker"""
        try:
            client = docker.from_env()
            image = client.images.build(path='.', tag='mlops-model')
            container = client.containers.run(image.id, detach=True, ports={'5000/tcp': 5000})
            logger.info(f"Model deployed in Docker container: {container.id}")
            return True
        except Exception as e:
            logger.error(f"Error deploying with Docker: {e}")
            return False
    
    def deploy_with_kubernetes(self, model: Any, config: Dict) -> bool:
        ...
        try:
            ...
            deployment = client.V1Deployment(
                metadata=client.V1ObjectMeta(name="mlops-model"),
                spec=client.V1DeploymentSpec(
                    replicas=1,
                    selector=client.V1LabelSelector(
                        match_labels={"app": "mlops-model"}
                    ),
                    template=client.V1PodTemplateSpec(
                        metadata=client.V1ObjectMeta(labels={"app": "mlops-model"}),
                        spec=client.V1PodSpec(
                            containers=[
                                client.V1Container(
                                    name="mlops-model",
                                    image="mlops-model:latest",
            ...
            logger.info("Model deployed in Kubernetes")
            return True
        except Exception as e:
...
class TrainingPipeline:
    ...
    def run(self) -> Dict:
        ...
        try:
            # Initialize stack 
            my_stack = Stack("ml_pipeline", max_workers=4, run='local', scale='docker')
